...
//		int[] n = new int[] { 10, 2, 11, 13, 1, 1, 1, 1, 1 };
		int[] n = new int[] { 2, 5, 1, 1, 1, 1, 4, 1, 7, 3, 7};
	...
	public static boolean f(int[] n, int[] sums, int start, int num, int t) {
		if (t == 1) // 已经是最后一个分组，此时只要判断该剩余数组是否满足 num即可。
			return sums[start] == num; //ssl:应该是sums[start] == num;
		...
		for (i = start; i < (n.length - 2 * (t - 1)) && s < num; i++) { //ssl:判断条件应该为：i<n.length-2*(t-1) && s<num;
			s += n[i];
		}
		// 该分组不满足构成num
		if (s != num) //ssl:条件应该为：s != num;
			return false;
